...
def main():
    ...
    parser.add_argument('--graph_pooling', type=str, default="mean",
                        help='Graph pooling mechanism')
    parser.add_argument('--L', type=int, default=200,
                        help='Number of simplex (default: 200)')
    parser.add_argument('--V', type=int, default=10,
                        help='Size of V (defaul: 10)')
    parser.add_argument('--tau', type=float, default=1,
                        help='SEM temperature (default: 1)')
    ...
    wandb.init(project='sem_gnn', name=args.name, config=args)
    ...
    if args.gnn == 'gin':
    ...
    elif args.gnn == 'gcn' and args.graph_pooling == 'sem':
        model = GNN(gnn_type='gcn', num_tasks=dataset.num_tasks, num_layer=args.num_layer, emb_dim=args.emb_dim, drop_ratio=args.drop_ratio, virtual_node=False,
                    graph_pooling=args.graph_pooling, L=args.L, V=args.V, tau=args.tau).to(device)
    elif args.gnn == 'gcn':
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        metrics = {
            f'train_{dataset.eval_metric}': train_perf[dataset.eval_metric],
            f'valid_{dataset.eval_metric}': valid_perf[dataset.eval_metric],
            f'test_{dataset.eval_metric}': test_perf[dataset.eval_metric],
        }
        wandb.log(metrics)
