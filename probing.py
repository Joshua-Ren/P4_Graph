...
#     return accuracy_score(y_pred, property[idx_test]), f1_score(y_pred, property[idx_test], average='macro'), roc_auc_score(property[idx_test], y_proba[:, 1], multi_class='ovr')


# scaled = embd[idx_train]
# scaled = scaled / (np.linalg.norm(scaled, axis=0)+1e-18)

# def ci(labels, all_clusters):
#     return np.array([cluster_impurity(labels, clusters) for clusters in all_clusters])

# def cluster_impurity(labels, clusters):
#     eye_labels = np.eye(labels.max()+1)[labels]
#     eye_clusters = np.eye(clusters.max()+1)[clusters]

#     freqs = eye_clusters.T @ eye_labels / eye_clusters.sum(0)[:, None]
#     purity = (freqs*np.log(freqs+1e-18)).sum(1)

#     return -purity.mean()
# def entropy(y):
#     counts = np.unique(y, return_counts=True)[1]
#     freqs = counts / counts.sum()
#     return (freqs*np.log2(freqs+1e-20)).sum()
